...
class RGBDPredictor:
    ...
    def __call__(self, rgb_image, depth_image):
        ...
        with torch.no_grad():
            ...
            depth_tensor = torch.as_tensor(depth_image.astype("float32").transpose(2, 0, 1))
...
def load_image_depth(image_path, input_type):
    ...
    # 对于纯rgb或depth输入，可以进行相应处理，这里主要关注rgbd
        ...
        # 创建一个全零的深度图作为占位符
        depth_image = np.zeros((rgb_image.shape[0], rgb_image.shape[1], 1), dtype=np.float32)
...
def main(args):
    ...
    colors = [
        ...
        (128, 128, 0), (128, 0, 128), (0, 128, 128), (255, 165, 0), (255, 20, 147)
    ]
    ...
    for i, mask in enumerate(masks):
        ...
        # 使用布尔型mask作为索引
        overlay[boolean_mask] = overlay[boolean_mask] * (1 - alpha) + np.array(color, dtype=np.uint8) * alpha
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-type", choices=["rgb", "rgbd"], default="rgbd", help="输入类型")
