...
def calculate_rgb_stats_for_dir(rgb_dir):
    ...
    try:
        image_files = [f for f in os.listdir(rgb_dir) if f.endswith(('.png', '.jpg', '.jpeg'))]
        if not image_files:
...
def main(args):
    ...
    for subset in subsets:
        ...
        if mean is not None and std is not None:
            ...
            print("MODEL:")
            print("  PIXEL_MEAN: [{:.4f}, {:.4f}, {:.4f}]".format(mean[0], mean[1], mean[2]))
            print("  PIXEL_STD: [{:.4f}, {:.4f}, {:.4f}]".format(std[0], std[1], std[2]))
            print("-" * 40)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="计算数据集中 train/test/val 子集的 RGB 均值和标准差")
    # 输入参数变为数据集根目录
    parser.add_argument('--dataset_root', type=str, default='/home/fuyx/zhn/mgm_datasets/0831_1K',
