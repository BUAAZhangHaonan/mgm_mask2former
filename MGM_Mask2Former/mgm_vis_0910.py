...
python3 MGM_Mask2Former/mgm_vis_0910.py \
  ...
  --rgb /home/fuyx/zhn/mgm_datasets/0909_512_0.12K/images/test/27,5x31x18x26_25_scene_000001_000001_v0.png \
  --depth /home/fuyx/zhn/mgm_datasets/0909_512_0.12K/depth/depth_npy/test/27,5x31x18x26_25_scene_000001_000001_v0.npy \
  --weights MGM_Mask2Former/pretrained-checkpoint/0909_512_0.12K_0909_60K.pth \
  --overlay-alpha 1.0
"""
...
def make_fake_batch(
    ...
    for _ in range(B):
        ...
        if with_noise_mask:
            sample["depth_noise_mask"] = (torch.rand(1, H, W, device=device) < depth_noise_ratio).float()
        if with_targets:
...
def count_parameters(model: nn.Module, detailed: bool = True) -> Dict:
    total_params = 0; trainable_params = 0; frozen_params = 0; module_stats = {}
    def fmt(n): return f"{n:,}"
    def mb(n): return n * 4 / (1024 * 1024)

    print("=" * 80); print("模型参数统计"); print("=" * 80)
    for name, p in model.named_parameters():
        n = p.numel(); total_params += n
        (trainable_params if p.requires_grad else frozen_params)  # just to satisfy linter
        if p.requires_grad: trainable_params += n
        else: frozen_params += n
        top = name.split(".")[0] if "." in name else "root"
        module_stats.setdefault(top, {"total":0,"trainable":0,"frozen":0,"params":[]})
        module_stats[top]["total"] += n
        (module_stats[top]["trainable"] if p.requires_grad else module_stats[top]["frozen"])
        if p.requires_grad: module_stats[top]["trainable"] += n
        else: module_stats[top]["frozen"] += n
        module_stats[top]["params"].append({"name":name,"shape":list(p.shape),"params":n,"trainable":p.requires_grad})
    ...
    if detailed:
        print("\n" + "=" * 80); print("各模块参数统计"); print("=" * 80)
        for mod, st in sorted(module_stats.items(), key=lambda x: x[1]["total"], reverse=True):
            print(f"\n📦 {mod}:")
...
def register_dummy_dataset(name: str, num_thing_classes: int = 3):
    if name in DatasetCatalog.list(): return
...
def build_cfg(yaml_path: str, device: Optional[str] = None):
    cfg = get_cfg(); add_mgm_config(cfg); cfg.merge_from_file(yaml_path)
    # dummy dataset
    train_name = "__dummy_rgbd_train__"; test_name = "__dummy_rgbd_test__"
    ...
    cfg.DATASETS.TRAIN = (train_name,); cfg.DATASETS.TEST = (test_name,)
...
# --------------------------------------------------------------------------------------
def load_rgb_depth(
    rgb_path: str, depth_path: str, device: str,
    resize: Optional[int] = None, keep_aspect: bool = True
    ...
    if resize is not None:
        if keep_aspect:
            w, h = rgb.size; scale = resize / float(max(w, h))
            rgb = rgb.resize((int(round(w*scale)), int(round(h*scale))), Image.BILINEAR)
        else:
...
def build_batch_from_files(
    rgb_path: str, depth_path: str, device: str,
    noise_mask: bool = True, resize: Optional[int] = None
) -> Tuple[List[Dict], Tuple[int,int], np.ndarray]:
    rgb_t, depth_t, hw, rgb_np = load_rgb_depth(rgb_path, depth_path, device=device, resize=resize)
    H, W = depth_t.shape[-2:]; assert tuple(rgb_t.shape[-2:]) == (H,W)
...
def save_confidence_and_overlays(
    ...
    def _save_one(name: str, map2d: torch.Tensor):
        ...
        Image.fromarray(colored).save(os.path.join(out_dir, f"{prefix}_conf_{name}.png"))
        if save_npy:
            np.save(os.path.join(out_dir, f"{prefix}_conf_{name}.npy"), map2d.cpu().numpy())
        # 叠加到原图（按当前读取尺寸作为“原始”）
        if do_overlay:
            ...
            map_resized = F.interpolate(map2d.unsqueeze(0).unsqueeze(0), size=(H0, W0), mode="bilinear", align_corners=False)[0,0]
            alpha_mask = (map_resized.clamp(0,1).cpu().numpy() * 255.0 * float(overlay_alpha)).astype("uint8")  # [H0,W0]
            hm_rgba = np.dstack([np.array(hm_img), alpha_mask])  # [H0,W0,4]
            base_rgba = np.dstack([base_rgb_np, np.full((H0, W0), 255, dtype=np.uint8)])
            out_rgba = Image.alpha_composite(Image.fromarray(base_rgba),
                                             Image.fromarray(hm_rgba)).convert("RGB")
...
def confidence_statistics(
    ...
    print("\n" + "=" * 80); print("MGM 置信度统计"); print("=" * 80)
    conf_dict = per_image_conf[0]
    agg = torch.stack([m[0] for m in conf_dict.values()])  # [S,H,W]
    mean_conf = agg.mean(0)

    depth = depths[0][0, 0]
    hole = ((depth <= z_min) | (depth >= z_max)).float(); valid = 1.0 - hole
    ...
    if noise_masks: print(f"  噪声: {conf_noise:.4f} | 干净: {conf_clean:.4f} | Δ={conf_clean-conf_noise:.4f}")
    print(f"  空洞: {conf_hole:.4f} | 有效: {conf_valid:.4f} | Δ={conf_valid-conf_hole:.4f}")
...
def main():
    ...
    parser.add_argument("--cfg", default="MGM_Mask2Former/configs/mgm_swin_convnext_tiny.yaml")
    parser.add_argument("--weights", default="MGM_Mask2Former/pretrained-checkpoint/0909_512_0.12K_0909_20K.pth")
    ...
    parser.add_argument("--out-dir", default="output/mgm_conf_out/0909_512_0.12K")
    parser.add_argument("--rgb", default="MGM_Mask2Former/predict_test/512/3521_7843763521_25_scene_000001_000001_v0.png")
    parser.add_argument("--depth", default="MGM_Mask2Former/predict_test/512/3521_7843763521_25_scene_000001_000001_v0.npy")
    ...
    # 叠加相关
    parser.add_argument("--overlay", action="store_true", help="生成叠加到原图的可视化热力图")
    parser.add_argument("--overlay-alpha", type=float, default=0.45, help="叠加透明度全局系数(0~1)")
    parser.add_argument("--overlay-inplace", action="store_true", help="叠加图保存到原始RGB所在目录")
    ...
    if has_weights:
        ...
        print(f"权重加载完成 | missing={len(ret.get('missing_keys',[]))} unexpected={len(ret.get('unexpected_keys',[]))}")
    ...
    if (not args.no_train) and (not force_single):
        ...
        batch = make_fake_batch(
            B=args.batch, H=args.size, W=args.size,
            with_targets=True, with_noise_mask=True,
            device=cfg.MODEL.DEVICE, depth_noise_ratio=args.depth_noise_ratio,
        )
        ...
        for s in batch:
            if s.get("_need_targets", False):
                ...
                gt.gt_classes = torch.randint(0, cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES, (n,), dtype=torch.int64)
                gt.gt_boxes = gt.gt_masks.get_bounding_boxes()
                s["instances"] = gt
                s.pop("_need_targets")
        losses = model(batch); loss_sum = sum(v for v in losses.values())
    ...
    if force_single:
        infer_inputs, hw, base_rgb_np = build_batch_from_files(
            args.rgb, args.depth, device=cfg.MODEL.DEVICE,
            noise_mask=True, resize=args.resize_input,
        )
    else:
        base_rgb_np = None
        infer_inputs = make_fake_batch(
            B=args.batch, H=args.size, W=args.size,
            with_targets=False, with_noise_mask=True,
            device=cfg.MODEL.DEVICE, depth_noise_ratio=args.depth_noise_ratio
        )
    ...
    with torch.inference_mode():
        ...
        per_image_conf, depths, noise_masks = extract_mgm_confidence(model, infer_inputs, upsample=True)

    # 统计只报 img0
    confidence_statistics(per_image_conf[:1], depths[:1], noise_masks[:1] if noise_masks else [], z_min=args.z_min, z_max=args.z_max)
    ...
    if args.export_conf:
        ...
        save_confidence_and_overlays(
            per_image_conf[:1], depths[:1], noise_masks[:1] if noise_masks else [],
            base_rgb_np=base_rgb_np if base_rgb_np is not None else (np.zeros((args.size,args.size,3),dtype=np.uint8)),
            out_dir=args.out_dir, prefix="img0", cmap=args.conf_cmap, save_npy=args.save_npy,
            do_overlay=args.overlay, overlay_alpha=args.overlay_alpha, overlay_inplace_dir=overlay_inplace_dir
        )
